...
## 🎯 FULL 4-PLATFORM REPORT GENERATED VIA CI (2026-07-14) — READ FIRST

**Complete consolidated report exists**, CI-generated by run 29354305288 (scratch c5c2970, platform=all):
- All 4 columns: mock, mi300x, strix-halo-linux, strix-halo-windows.
- Version headings LIVE (OS/ROCm 7.13.0/vLLM 0.23.0+rocm723/lemonade 10.6.0) — task #23 feature working in CI.
- Reconciliation: mock 4xfail/0XPASS/0unexp ✅; mi300x 11/0/0 ✅; strix-ubuntu 11/0/0 ✅;
  strix-windows 5xfail/1XPASS/0unexp (the chat-tool-definitions XPASS).
- Report saved: workspace/wip/ci-full/index.html (157KB, CI artifact).

**Key fixes this stretch (all on scratch, pushed):**
- CONFIRMED CI can run + generate a report (mock-only run 29343519419 first, then built out).
- os=linux scoping for ALL 6 lemonade xfails: they pass on native Windows (only fail on Linux
  Vulkan). Fixed 5 in 2533b4f, the last (chat-tool-definitions) in e21297d.
- runtime-install-sdk-active tagged @nightly (c5c2970): its cold full-devel install takes ~35min
  (devel-tar unpack + find_libraries probe on the overlay), nearly capping the 90min per-PR GPU
  job. Now excluded from fast per-PR run; runs in nightly. FOLLOW-UP: speed up cold devel install.
- versions feature (task #23): harness collect_versions() reads OS(examine)/ROCm(runtimes list)/
  vLLM(dist-info)/lemonade(lemond --version) into platform.json; report renders per-column.
- IMPORTANT process: container gate MUST use RUSTFLAGS="-D warnings" (CI does) — a mock-lib
  unused-var slipped to CI once (f70cc92 fixed it).

**Scratch HEAD e21297d. Confirming strix-windows 0 XPASS via run 29356345254.**
**TWO @nightly scenarios now:** serve-large-model-inference (Qwen3.6-27B, user priority) +
runtime-install-sdk-active. Nightly must serve the 27B AND do the cold install under 90min cap.

**NOT yet on PR branch:** the version feature, os=linux scoping, @nightly-install-tag are on
SCRATCH only (e21297d). Bring to PR (test/add-e2e-robot-framework) after strix-windows confirms.

---

# WIP: E2E BDD tests for rocm-cli (PR #69, cucumber-rs)

**Stage:** 26-report-polish-version-probe-in-progress-nightly-validation
**Pipeline:** standard
**Branch:** test/add-e2e-robot-framework (PR-side) / ci-e2e-framework-fixes (scratch, active)
**Last Updated:** 2026-07-14
**Token Usage:** in=13226 out=4222395 cache_create=50129798 cache_read=2401925838 calls=6637

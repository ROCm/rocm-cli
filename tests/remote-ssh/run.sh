...
# Read without `|| true`: an unreadable file yields an empty string, which
# trivially does not contain the key, so the absence check below would pass
# having verified nothing. This is the check backing the claim that a credential
# never reaches a process table, so it has to fail loudly when it cannot look.
recorded_argv="$(remote cat /var/lib/fake-rocm/last-argv)"
if [[ -z "${recorded_argv}" ]]; then
  fail "and never appeared in the command line" \
    "could not read /var/lib/fake-rocm/last-argv, so nothing was actually checked"
elif printf '%s' "${recorded_argv}" | grep -q 'sekrit-key'; then

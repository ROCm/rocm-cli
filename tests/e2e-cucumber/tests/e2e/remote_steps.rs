...
async fn then_doctor_installed_nothing(world: &mut E2eWorld) {
    let said = said(world);
    assert_ne!(
        world.cli_rc,
        Some(0),
        "a health check with no CLI must fail:\n{said}"
    );
    // No "or it never got there" alternative: this scenario's Given starts a
    // real container, so the machine is reachable by construction. Accepting
    // the unreachable wording too would let the scenario pass having checked
    // only that the command failed somehow — and it would keep passing if the
    // container stopped coming up.
    assert!(
        said.contains("only reads"),
        "a read-only command must say it changed nothing:\n{said}"
    );
}

#[then("they are pointed at serving as the command that provisions")]
async fn then_doctor_points_at_serve(world: &mut E2eWorld) {
    let said = said(world);
    // Asserted unconditionally, for the same reason as above: guarding this on
    // the output already containing "only reads" made the step vacuous on every
    // other path, so an unexpected one passed silently instead of failing.
    assert!(
        said.contains("rocm remote serve"),
        "the health check must name serving as what provisions the machine:\n{said}"
    );
}
...
impl RemoteMachine {
    ...
    /// A failed command is a failed test, not an empty string. The callers of
    /// this assert on what the container reports — including
    /// `then_publishing_nothing`, which asserts a port is *absent* from the serve
    /// config. That assertion passes trivially on empty output, so a broken
    /// container or a missing binary would read as "the endpoint was withdrawn"
    /// and quietly retire the one check that does not take the CLI's word for it.
...
///
/// Only a candidate: the listener is closed before the caller uses the port, so
/// something else can take it in between. [`start_on_a_free_port`] is what makes
/// that survivable.
fn free_port() -> u16 {
...
/// Start the stand-in machine on a loopback port, retrying if the port was
/// taken between picking it and publishing it.
///
/// Returns the container name and the port it is reachable on.
fn start_on_a_free_port(tag: &str) -> (String, u16) {
    ...
    for _ in 0..10 {
        ...
        last_error = String::from_utf8_lossy(&run.stderr).into_owned();
        // A failed `run` can still leave the named container behind, which
        // would make every retry fail on the name rather than the port.
        let _ = docker(&["rm", "-f", &container]);
    }
    panic!("failed to start the stand-in machine after 10 attempts: {last_error}");
}
...
fn start_remote_machine_variant(world: &E2eWorld, include_rocm_cli: bool) -> RemoteMachine {
    ...
    let root = world
        ...
        .path()
        .to_path_buf();
    // Retried rather than assumed: `free_port` closes its listener before
    // docker binds, and this lane runs scenarios 64-way concurrent, so the
    // port can be taken in between. Letting docker pick (`-p 127.0.0.1:0:22`)
    // would close the gap outright, but `tests/remote-ssh/run.sh` documents
    // that some Docker setups do not forward ephemeral publications to the
    // host at all — so the explicit port stays and the race is handled by
    // trying again with a different one.
    let (container, port) = start_on_a_free_port(tag);

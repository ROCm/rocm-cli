...
async fn then_doctor_installed_nothing(world: &mut E2eWorld) {
    ...
    assert!(
        said.contains("only reads") || said.contains("could not reach"),
        "a read-only command must say it changed nothing, or that it never got there:\n{said}"
    );
}
...
async fn then_doctor_points_at_serve(world: &mut E2eWorld) {
    let said = said(world);
    // Only meaningful when the machine was reachable enough to answer; an
    // unreachable one is a different, earlier failure that says so itself.

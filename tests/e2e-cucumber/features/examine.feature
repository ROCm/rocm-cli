Feature: GPU detection and system inspection
  ...
  # and where KFD's topology is readable also checks that each record carries
  # the target the kernel reports for that PCI address — a target present on
  # the wrong GPU (rocminfo and lspci enumerating in different orders) is not
  # a pass.

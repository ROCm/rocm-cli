...
Use this workflow for any work intended for repositories the company does not own.
...
- use company author/committer identity
...
- do not include internal names, aliases, internal URLs, internal hostnames, internal gateways, internal cluster/namespace names, internal registry paths
- do not include internal Jira/ticket identifiers, Confluence links, or unrelated internal usernames
...
Use neutral external framing (for example: "backend" or "gateway") rather than internal ownership phrasing.

Leak scan before each upstream push/PR/comment batch:

```bash
git diff <upstream-base>..HEAD \
  | grep -inE 'amd|silogen|xilinx|\.amd\.com|jira|confluence|internal|\bvsr\b' \
  && echo "REVIEW each hit" || echo "diff clean"
```

Also manually review non-diff text surfaces (PR body, comments, issue text, branch name).
...
- follow sync notes in `docs/upstream-codex-sync.md` for pin updates and rebuild workflow
...
7. Company identity, DCO sign-off, and commit-signing requirements preserved — *§2*

...
pub(crate) fn resolve_peer<'a>(
    ...
    // Collected rather than `find`-ed. `matches_exactly` compares on `host`,
    // which is documented above as not unique across a tailnet — two machines
    // sharing an OS hostname is ordinary, since MagicDNS dedupes `DNSName` and
    // not `HostName`. Taking the first would be exactly the silent guess the doc
    // comment above forbids, and the loose path below already refuses it.
    let exact = status
        .peers
        .iter()
        .filter(|peer| peer.matches_exactly(needle))
        .collect::<Vec<_>>();
    match exact.as_slice() {
        [] => {}
        [only] => return Ok(Some(only)),
        several => return Err(ambiguous(needle, several)),
    }
    ...
    match loose.as_slice() {
        [] => Ok(None),
        [only] => Ok(Some(only)),
        several => Err(ambiguous(needle, several)),
    }
}

/// The refusal both match passes share: name the candidates rather than pick one.
fn ambiguous(needle: &str, candidates: &[&TailnetPeer]) -> anyhow::Error {
    anyhow::anyhow!(
        "`{needle}` matches more than one machine on the tailnet: {}\n\
         Name one of them exactly.",
        candidates
            .iter()
            .map(|peer| peer.dns_name.as_str())
            .collect::<Vec<_>>()
            .join(", ")
    )
}
...
pub(crate) fn render_targets(status: &TailnetStatus, tag: Option<&str>) -> String {
    ...
    for peer in peers {
        ...
        // Quoted, because this line is offered to be copied and run and the name
        // in it is not ours. `HostName` comes straight from the peer's own
        // `tailscale status` entry — any device on the tailnet chooses its own —
        // so an unquoted one renders a command that does something other than
        // what it reads as, on the machine of whoever pastes it.
        let _ = writeln!(
            output,
            "  check: rocm remote doctor {}",
            super::shell_quote(&peer.host)
        );
    }
...
mod tests {
    ...
    #[test]
    fn two_machines_sharing_a_host_name_are_refused_not_silently_picked() {
        // `HostName` is not unique on a tailnet — MagicDNS dedupes `DNSName`,
        // not the OS hostname — so two machines both called `gpu-box` is
        // ordinary, not contrived. Both match *exactly*, which is the path that
        // used to take the first and serve a model on whichever peer happened to
        // sort first.
        const DUPLICATE_HOSTS: &str = r#"{
          "Version": "1.999.0",
          "TUN": true,
          "BackendState": "Running",
          "MagicDNSSuffix": "example-tailnet.ts.net",
          "Self": {"ID": "nS", "HostName": "laptop",
                   "DNSName": "laptop.example-tailnet.ts.net.", "OS": "linux",
                   "TailscaleIPs": ["100.88.0.9"], "Online": true},
          "Peer": {
            "nodekey:aaa": {"ID": "nA", "HostName": "gpu-box",
                            "DNSName": "gpu-box.example-tailnet.ts.net.", "OS": "linux",
                            "TailscaleIPs": ["100.88.14.21"], "Online": true},
            "nodekey:bbb": {"ID": "nB", "HostName": "gpu-box",
                            "DNSName": "gpu-box-2.example-tailnet.ts.net.", "OS": "linux",
                            "TailscaleIPs": ["100.88.14.37"], "Online": true}
          }
        }"#;
        let status = parse_status(DUPLICATE_HOSTS).expect("fixture parses").1;

        let error = resolve_peer(&status, "gpu-box")
            .expect_err("an exactly-ambiguous name must be refused, not guessed")
            .to_string();
        // Named by DNS name, not host name: printing the colliding host twice
        // would tell the user nothing they could act on.
        assert!(error.contains("gpu-box.example-tailnet.ts.net"), "{error}");
        assert!(
            error.contains("gpu-box-2.example-tailnet.ts.net"),
            "{error}"
        );

        // And a name that singles one of them out still resolves.
        let peer = resolve_peer(&status, "gpu-box-2.example-tailnet.ts.net")
            .expect("an unambiguous exact name resolves")
            .expect("the peer is present");
        assert_eq!(peer.dns_name, "gpu-box-2.example-tailnet.ts.net");
    }
    ...
    #[test]
    fn a_peer_name_carrying_shell_syntax_is_quoted_before_it_is_offered_to_run() {
        // Every device on a tailnet chooses its own `HostName`, so this string is
        // not ours. It is rendered into a line the user is invited to copy, which
        // makes an unquoted one a command that does something other than what it
        // reads as — on the machine of whoever pastes it.
        const HOSTILE: &str = r#"{
          "Version": "1.999.0", "TUN": true, "BackendState": "Running",
          "MagicDNSSuffix": "example-tailnet.ts.net",
          "Self": {"ID": "nS", "HostName": "laptop",
                   "DNSName": "laptop.example-tailnet.ts.net.", "OS": "linux",
                   "TailscaleIPs": ["100.88.0.9"], "Online": true},
          "Peer": {
            "nodekey:aaa": {"ID": "nA", "HostName": "box; rm -rf ~",
                            "DNSName": "box.example-tailnet.ts.net.", "OS": "linux",
                            "TailscaleIPs": ["100.88.14.21"], "Online": true}
          }
        }"#;
        let status = parse_status(HOSTILE).expect("fixture parses").1;
        let rendered = render_targets(&status, None);

        assert!(
            !rendered.contains("doctor box; rm -rf ~"),
            "a peer name must not reach the advice line unquoted:\n{rendered}"
        );
        // Ask a real shell what the rendered line would actually pass along.
        let line = rendered
            .lines()
            .map(str::trim)
            .find(|line| line.starts_with("check: "))
            .expect("every peer carries a check line")
            .trim_start_matches("check: ")
            .to_owned();
        let output = std::process::Command::new("sh")
            .arg("-c")
            .arg(format!("rocm() {{ printf %s \"$3\"; }}\n{line}"))
            .output()
            .expect("sh should run");
        assert_eq!(
            String::from_utf8_lossy(&output.stdout),
            "box; rm -rf ~",
            "the name must arrive as one literal argument"
        );
    }

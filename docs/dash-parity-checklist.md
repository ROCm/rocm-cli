...
|---|-------------------|----------------|----------------|--------|----------|
| 1 | Read-only inspection (doctor, engines, services, gpu, model, config, logs, daemon, runtimes, paths) | `tui.rs` read-only tool-calls + slash commands | Phase 3 read-only `rocm_command` tools via the bin-side seam (`dash_seam::BinToolExecutor`) + nav/overlay slash commands | ACCEPTED | `read_only_tool_round_trips_to_json` (agent); `slash_doctor_opens_overlay`, `slash_runtimes_opens_overlay`, `slash_gpu_switches_to_observe`, `slash_daemon_raises_executor_request` (app); map rows B |

...
def therock_sdk_package_spec(include_devel: bool) -> str:
    """The `rocm` wheel spec prefix `rocm install sdk` is expected to plan for.

    The compiler toolchain is opt-in, so a default install asks for the runtime
    libraries alone and only `--devel` adds `devel`.

    A prefix, not the whole spec: the canonical index always appends a further
    extra naming the device payload (`rocm[libraries,devel,device-gfx1201]`), so
    the closing bracket is not in a fixed place.
    """
    return "rocm[libraries,devel," if include_devel else "rocm[libraries,device-"

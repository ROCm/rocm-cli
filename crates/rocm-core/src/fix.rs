...
mod tests {
    ...
    /// detect-but-cannot-repair failure be accepted as an entry at all, and
    /// adding it after the manifest ships would be a type change that raises the
    /// contract version. This test is the tripwire — when it fails, the first
    /// real member has arrived, and the Epic's scenario 10 becomes demonstrable
    /// against it rather than staying provisional.
    #[test]
